...
`url = "https://en.wikipedia.org/wiki/Production_car_speed_record"` <br>
`data = requests.get(url).text` <br>
`soup = BeautifulSoup(data, 'html.parser')` <br>

Now let's take a look at the tables available to use in the soup. We are soon going to want information about the class of the table, so let's print it out. Here is the code along with the results. <br>
